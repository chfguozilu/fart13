...
static inline JValue Execute(
  ...
  // Gate 2: by this point ART has built the interpreter frame and any shell
  // sitting on the invoke/bridge path has had a chance to replace data_. Copy
  // the runtime CodeItem and return before ExecuteSwitchImpl executes even one
  // instruction from the target method.
  ArtMethod* const probe_target = shadow_frame.GetMethod();
  if (UNLIKELY(runtime_snapshot::GetProbeTarget() == probe_target)) {
    runtime_snapshot::CaptureMethod(
        probe_target, runtime_snapshot::CaptureStage::kSwitchInterpreterEntry);
    JValue zero_result;
    zero_result.SetJ(0);
    return zero_result;
  }
...
void EnterInterpreterFromInvoke(Thread* self,
  ...
  // ArtMethod::Invoke() reaches this function directly when interpretation is
  // forced. Stop an exact TLS-selected probe before decoding a receiver,
  // building a ShadowFrame, or calling EnsureInitialized() for a static method.
  // In particular, this makes receiver-less <init> probing safe in this path
  // and prevents the probe from changing class initialization state.
  if (UNLIKELY(runtime_snapshot::GetProbeTarget() == method)) {
    runtime_snapshot::CaptureMethod(
        method, runtime_snapshot::CaptureStage::kInterpreterFromInvoke);
    if (result != nullptr) {
      result->SetJ(0);
    }
    return;
