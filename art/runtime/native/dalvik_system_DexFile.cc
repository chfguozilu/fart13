...
#include "runtime.h"
#include "runtime_asm_entrypoints.h"
...
class ScopedRuntimeInterpreterEntry final {
 public:
  explicit ScopedRuntimeInterpreterEntry(ArtMethod* method)
      : method_(method),
        previous_(method->GetEntryPointFromQuickCompiledCode()),
        replaced_(false) {
    // A base APK may already have AOT/JIT quick code. Replace entry points that ART itself can
    // identify. Preserve an unknown/custom entry point because it may be the
    // shell trampoline that must run in order to release the real CodeItem.
    const void* const oat_code = method_->GetOatMethodQuickCode(kRuntimePointerSize);
    ClassLinker* const class_linker = Runtime::Current()->GetClassLinker();
    jit::Jit* const jit = Runtime::Current()->GetJit();
    const bool is_jit_code =
        jit != nullptr && jit->GetCodeCache()->ContainsPc(previous_);
    // A genuine Nterp entry is intentionally preserved: arm/arm64 main.S now
    // performs the late capture and safe early return itself. Known compiled
    // code is still redirected to q2i so active probes cannot execute it.
    if (previous_ == nullptr ||
        previous_ == GetQuickToInterpreterBridge() ||
        class_linker->IsQuickResolutionStub(previous_) ||
        previous_ == oat_code ||
        is_jit_code) {
      method_->SetEntryPointFromQuickCompiledCode(GetQuickToInterpreterBridge());
      replaced_ = true;
    }
  }

  ~ScopedRuntimeInterpreterEntry() {
    // Do not overwrite an entry point deliberately replaced by a shell while
    // the probe was in progress.
    if (replaced_ &&
        method_->GetEntryPointFromQuickCompiledCode() == GetQuickToInterpreterBridge()) {
      method_->SetEntryPointFromQuickCompiledCode(previous_);
    }
  }

  ScopedRuntimeInterpreterEntry(const ScopedRuntimeInterpreterEntry&) = delete;
  ScopedRuntimeInterpreterEntry& operator=(const ScopedRuntimeInterpreterEntry&) = delete;

 private:
  ArtMethod* const method_;
  const void* const previous_;
  bool replaced_;
};

bool IsRuntimeProfileEligibleMethod(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
...
void ClearRuntimeProfileException(Thread* self, ArtMethod* method)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (self->IsExceptionPending()) {
    LOG(INFO) << "[RuntimeProfile] probe exception from " << method->PrettyMethod()
              << ": " << self->GetException()->Dump();
    self->ClearException();
  }
}

void InvokeRuntimeStaticMethod(Thread* self, ArtMethod* method)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  const char* shorty = method->GetShorty();
  const size_t argument_registers = ArtMethod::NumArgRegisters(shorty);
  std::vector<uint32_t> args(argument_registers, 0u);
  JValue result;
  result.SetJ(0);

  {
    ScopedRuntimeInterpreterEntry interpreter_entry(method);
    runtime_snapshot::ScopedProbeTarget probe(method);
    method->Invoke(self,
                   args.empty() ? nullptr : args.data(),
                   static_cast<uint32_t>(args.size() * sizeof(uint32_t)),
                   &result,
                   shorty);
  }
  ClearRuntimeProfileException(self, method);
}

void InvokeRuntimeConstructor(Thread* self, ArtMethod* method)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  const char* shorty = method->GetShorty();
  const size_t argument_registers = ArtMethod::NumArgRegisters(shorty) + 1u;
  // The first vreg is the receiver. It deliberately stays null: all supported
  // runtime paths have an exact-target gate before receiver decoding or target
  // bytecode execution. Avoiding AllocObject() lets constructors of abstract
  // and finalizable classes be captured without creating a malformed object
  // that a later finalizer could process.
  std::vector<uint32_t> args(argument_registers, 0u);
  JValue result;
  result.SetJ(0);
...
void InvokeRuntimeInstanceMethod(Thread* self,
                                 ArtMethod* method,
                                 Handle<mirror::Object> receiver)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  const char* shorty = method->GetShorty();
  const size_t argument_registers = ArtMethod::NumArgRegisters(shorty) + 1u;
  std::vector<uint32_t> args(argument_registers, 0u);

  // Recreate the compressed reference immediately before every invocation.
  // receiver is a Handle, so receiver.Get() follows a moving GC.
  const mirror::CompressedReference<mirror::Object> receiver_reference =
      mirror::CompressedReference<mirror::Object>::FromMirrorPtr(receiver.Get());
  args[0] = receiver_reference.AsVRegValue();
...
// The Java ParcelFileDescriptor remains owned by Java. StartWriter() dup()s
// this descriptor and ART closes only its duplicate in StopWriter().
static jboolean DexFile_beginRuntimeProfile(JNIEnv*, jclass, jint fd) {
  return runtime_snapshot::StartWriter(fd) ? JNI_TRUE : JNI_FALSE;
}

static jboolean DexFile_endRuntimeProfile(JNIEnv*, jclass) {
  return runtime_snapshot::StopWriter() ? JNI_TRUE : JNI_FALSE;
}

static void DexFile_visitRuntimeMethods(JNIEnv* env, jclass, jclass target) {
  if (target == nullptr) {
    return;
  }
  if (!runtime_snapshot::IsWriterStarted()) {
    LOG(ERROR) << "[RuntimeProfile] refusing to probe without an active writer";
    return;
  }

  ScopedObjectAccess soa(env);
  Thread* self = soa.Self();
  ObjPtr<mirror::Class> klass = soa.Decode<mirror::Class>(target);
  if (klass == nullptr) {
    return;
  }

  StackHandleScope<2> handles(self);
  Handle<mirror::Class> h_klass = handles.NewHandle(klass);

  // ArtMethod storage is native/non-moving. The Class itself is kept through
  // h_klass because any Invoke below may trigger a moving GC.
  ArraySlice<ArtMethod> methods = h_klass->GetDeclaredMethods(kRuntimePointerSize);

  // Probe <clinit> explicitly before ordinary static methods.  The scoped
  // entry-point override recognizes ART's quick-resolution stub and sends it
  // straight to q2i; the q2i probe gate then captures before EnsureInitialized.
  // Consequently this direct probe does not run the class initializer and does
  // not change the declaring class' initialization status.
  for (ArtMethod& method : methods) {
    if (IsRuntimeProfileEligibleMethod(&method) && method.IsClassInitializer()) {
      InvokeRuntimeStaticMethod(self, &method);
    }
  }

  // Static methods need no receiver. Exclude <clinit> here because it was
  // handled by the dedicated loop above.
  for (ArtMethod& method : methods) {
    if (IsRuntimeProfileEligibleMethod(&method) &&
        method.IsStatic() &&
  ...
  // <init> is a direct instance method but it does not need a constructed
  // receiver for capture: every route is stopped before receiver use. Keeping
  // this loop before the allocation checks also covers constructors declared
  // by abstract and finalizable classes.
  for (ArtMethod& method : methods) {
    if (IsRuntimeProfileEligibleMethod(&method) &&
        method.IsConstructor() &&
        !method.IsStatic()) {
      InvokeRuntimeConstructor(self, &method);
    }
  }

#if defined(ART_TARGET_ANDROID)
  // Instance probing is device-only. This translation unit is also linked into
  // host dex2oatd, whose link graph does not provide Class::AllocObject(). A
  // compile-time guard is required; a runtime "if" would leave the undefined
  // symbol reference in the host object file.
  //
  // A raw receiver can only be created for a concrete, non-array class. This
  // allocation intentionally does not execute a Java constructor.
  if (!h_klass->IsInstantiableNonArray()) {
    LOG(INFO) << "[RuntimeProfile] skip instance methods of non-instantiable class "
              << h_klass->PrettyDescriptor();
    return;
  }

  // AllocObject() does not execute Java constructors. If the class (including
  // an inherited superclass) is finalizable, ART registers the raw object for
  // finalization. A later FinalizerDaemon invocation can then enter native
  // cleanup with uninitialized native fields (for example SurfaceTexture) and
  // crash the process. Static methods above are still probed; only unsafe raw
  // receiver probing is skipped here.
  if (h_klass->IsFinalizable()) {
    LOG(INFO) << "[RuntimeProfile] skip instance methods of finalizable class "
              << h_klass->PrettyDescriptor();
    return;
  }

  Handle<mirror::Object> h_receiver = handles.NewHandle(h_klass->AllocObject(self));
  if (h_receiver == nullptr) {
    LOG(INFO) << "[RuntimeProfile] raw receiver allocation failed for "
              << h_klass->PrettyDescriptor();
    if (self->IsExceptionPending()) {
      LOG(INFO) << "[RuntimeProfile] allocation exception: " << self->GetException()->Dump();
      self->ClearException();
    }
    return;
  }

  // Finally probe ordinary instance methods; <init> was handled before the
  // allocation checks and is excluded here to avoid a duplicate record.
  for (ArtMethod& method : methods) {
    if (IsRuntimeProfileEligibleMethod(&method) &&
        !method.IsStatic() &&
        !method.IsConstructor()) {
      InvokeRuntimeInstanceMethod(self, &method, h_receiver);
    }
  }
#endif
}
...
static JNINativeMethod gMethods[] = {
  ...
  NATIVE_METHOD(DexFile, beginRuntimeProfile, "(I)Z"),
  NATIVE_METHOD(DexFile, endRuntimeProfile, "()Z"),
  NATIVE_METHOD(DexFile, visitRuntimeMethods, "(Ljava/lang/Class;)V")
};
