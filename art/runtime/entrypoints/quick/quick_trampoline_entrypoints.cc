...
extern "C" uint64_t artQuickToInterpreterBridge(ArtMethod* method, Thread* self, ArtMethod** sp)
  ...
  // Active runtime probes must stop before the class-initialization check below.  Reaching this
  // bridge means that any custom quick trampoline in front of ART has already had an opportunity
  // to restore/replace method->data_.  Capturing here therefore preserves the useful late snapshot
  // without executing <clinit>, without marking an uninitialized class as initialized, and without
  // executing the first instruction of the target method.  The TLS target comparison keeps normal
  // application calls completely unchanged.
  if (UNLIKELY(runtime_snapshot::GetProbeTarget() == method)) {
    runtime_snapshot::CaptureMethod(
        method, runtime_snapshot::CaptureStage::kQuickToInterpreterBridge);
    return 0u;
  }
